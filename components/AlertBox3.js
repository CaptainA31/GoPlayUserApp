...
const AlertBox3 = ({ alertBox, setAlertBox, cancel, success, message, title, onSuccess, onAbort }) => {
        ...
        <View style={{ width: "75%", borderRadius: 15, overflow: "hidden", alignSelf: "center", backgroundColor: "white" }}>
          ...
          <View style={{ flexDirection: "row", borderTopWidth: 0.8, borderColor: colors.shade, justifyContent: "space-between", paddingVertical: "4%" }}>
            <   TouchableOpacity
              ...
              <Text style={{ fontSize: RFValue(14), fontWeight: "bold" }}>{cancel ? cancel : "No"}</Text>
            </TouchableOpacity>
            <TouchableOpacity
              onPress={() => onAbort()}
              style={{ width: "50%", alignItems: "center" }}>
              <Text style={{ fontSize: RFValue(14), fontWeight: "bold" }}>{success ? success : "Yes"}</Text>
            </TouchableOpacity>
          </View>
