...
import { colors, imageStyle, stringReduce } from '../GlobalStyles'
...
import { calculateDuration } from '../services/HostActivity'
import { webURL } from '../services/BaseURL'
import { useDispatch , useSelector } from 'react-redux';

const Games = ({hide , myGameList , handleJoinGame , handleLeaveGame , disable , navigation , isPast}) => {
    const userDetail = useSelector(state => state?.userDetail.userDetail)
    const alreadyJoined = (element)=> { 
        const exist = element?.playerJoined?.find(item => item._id == userDetail._id )
        if (exist) {
            return true
        }else {
            return false
    ...
    return (
        ...
        >
            <View style = {[styles.cardContainer , {height : hide ? RFValue(140) : RFValue(175)}]}>
                <TouchableOpacity
                onPress = {()=> navigation.navigate("summary" , {myGameList : myGameList})}
                style={styles.card}>
            {
                hide &&    
                 <View style = {styles.date}>
                <Text style={{color: "white" ,fontSize :RFValue(10) }}>Activity</Text>
                </View>
            }
               ...
               <Text style = {{fontSize : RFValue(12), fontWeight : "bold"}}>Organizer</Text>
                ...
                <View style={{width : "70%"  , height : "100%" , paddingTop : "7%"}}>
                    <Text numberOfLines = {1} style = {{fontSize : RFValue(15)}}>{myGameList?.activityName }</Text>
                    ...
                    <View style={{flexDirection : "row" , marginTop : "2%" , width : "85%", justifyContent :"space-between"}}>
                        ...
                        <Text style={{fontSize : RFValue(9)}}>{myGameList?.startTime}- {myGameList?.endTime}</Text>
                        </View>
                    </View>
                    ...
                    <View style = {{flexDirection : "row" , marginTop : "2%"}}>
                        ...
                        <Text style={{fontSize : RFValue(9)}}>{calculateDuration(myGameList?.startTime , myGameList?.endTime)}</Text>
                        </View>
                        ...
                        <Text style={{fontSize : RFValue(10)}}>Total : {myGameList?.total} | Confirmed: {myGameList?.confirmed} {myGameList?.total == myGameList.confirmed && " (Full)"}</Text>
                        </View>
